...
def getLines(map_path):
#    //FILE PATH TO CORRECT //
  fh = open(str(map_path), "r")
  lines = fh.readlines()
  return lines
#Initialise  Maplist
def initialiseMapXY(map_path):
  MapXY=[]
...
def updateGhostsPannel(Players,MapXY):
  for player in [1,2]:
      ...
      for ghost in range(len(Players[player]['ghostsLocationList'])):
        print(term.move_xy(startprint,ghost+7)+term.white3+'•'+str(Players[player]['ghostsLocationList'][ghost][0])+'-'+str(Players[player]['ghostsLocationList'][ghost][1])+':'+str(Players[player]['ghosts'][Players[player]['ghostsLocationList'][ghost]])+'%'+term.normal+term.move_xy(190,190),end='',flush=True)
...
def isCaseFree(Players,futuple):
   ...
   if futuple in allghostsLocation or futuple in allPlayersPopLocation:
      free = False
   return free
def moveGhost(Players,PlayerID,currtuple,futuple,ghostsdone,movementflag):
...
def getOrderslist(MapXY,type_1,type_2, Players,MagicDict,MagicList):
    orderslistP1P2=[]
    if type_1 == type_2 and type_1 == "human":
      ...
      return orderslistP1P2
       ...
       for player in [1,2]:
        ...
        orderslistP1P2.append(orders.split())    
       return orderslistP1P2   
def Invokeghost (Players,PlayerID,ghostsdone):
...
def Healghost(Players,PlayerID,targetGhost,amount,ghostsdone):
...
def transformOrder (splitted):
    ...
    return fulltuple 
def grantMagic(Players,PlayerID,from_to,MagicList,MagicDict,ghostsdone):
...
def moveVisual(Players,PlayerID,currtuple,futuple):
   if (currtuple[0]+currtuple[1])%2 ==0 :
     print(term.move_xy(25+currtuple[0]*2,currtuple[1])+term.on_white+'  '+term.normal+term.move_xy(50,50),end ='',flush=True)
   else:
   ...
   return  None
def reEstablishCase(Players,MapXY,case,MagicDict,MagicList):
   if case in MagicList:
      if (case[0]+case[1])%2 == 0 :
            print(term.move_xy(25+case[0]*2,case[1])+term.on_white+returnCorrectMagicEmoji(MagicDict[case])+term.normal+term.move_xy(50,50),end='',flush=True)
      else :
            print(term.move_xy(25+case[0]*2,case[1])+term.on_red3+returnCorrectMagicEmoji(MagicDict[case])+term.normal+term.move_xy(50,50),end='',flush=True)
     ...
     for i in [0,1]:
      for order in Correctorrders[i]:
        ...
        if splitted[1][0] == "$" :
          ...
          updateMagicDisplay(Players,MapXY)
def Phase4(Correctorrders,Players,MapXY,ghostsdone,MagicDict,MagicList,deathflag):
     P1P2attackOrders=[]
     for i in [0,1]:
      for order in Correctorrders[i]:
        splitted=order.split(':')
        ...
        for i in [0,1]:
          for case in P1P2attackOrders:
            ...
            updateGhostsPannel(Players,MapXY)
            updateGhostsNumberDisplay(Players,MapXY)     
def Phase5(Correctorrders,Players,MapXY,ghostsdone,MagicDict,MagicList,movementflag):
     for i in [0,1]:
      for order in Correctorrders[i]:
        movementflag == False
        splitted=order.split(':')
...
def Phase6(Players,MapXY):
     ...
     updateMagicDisplay(Players,MapXY)
def aiOrders(Players,PlayerID,MapXY,MagicDict,MagicList):
    aiOrdersString =""                             
...
def displayScoreGhostZero(Players,MapXY):
   print(term.home+term.clear)
   if len(Players[2]["ghostsLocationList"]) == 0 : 
     ...
     print(term.move_xy(round(term.width/3),round(term.height/3)),"𝕋H𝔼 𝕎𝕀NN𝔼R 𝕀𝕊 : P𝕃𝔸𝕐𝔼R 𝕋𝕎𝕆  🧛  \n" +term.move_xy(round(10),round(term.height/2))+ str(len(Players[2]["ghostsLocationList"]))+"  -  " +str(len(Players[1]["ghostsLocationList"]))+term.normal+term.move_xy(50,50),end ='',flush=True)
      ...
      print("𝔻R𝔸𝕎") 
def displayScore(Players,MapXY):
   print(term.home+term.clear)
...
#Initialise MagicDict and Magiclist
  ...
  while game_over == False:
    ...
    print(term.move_xy(round(100),round(100))+str(Correctorrders),end ='',flush=True)
     #GHOST BUYING PHASE
    ...
    if tourcounter == 1000 and deathflag == False :
